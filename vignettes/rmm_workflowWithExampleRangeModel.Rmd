...
pt <- ENMeval::maxnet.predictRaster(m, pt.ext, type = "exponential", doClamp = TRUE)
plot(pt)
```
...
## Model Fitting 
```{r}
rmm$modelFit$objective$purpose <- 'projection'
rmm$modelFit$objective$rangeType <- 'potential'
rmm$modelFit$objective$transfer <- 'predict suitable areas without current sampling data'
```
