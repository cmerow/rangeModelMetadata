...
p <- e@predictions[[i]]
plot(p)
# alternatively, you could make a prediction transformation
# bounded by 0-1 -- the choices are "logistic" or "cloglog" 
# (the default of Maxent 3.4.x -- see Phillips et al. 2017
# [Ecography])
p.cloglog <- predict(m, bg.ext, args=c("outputformat=cloglog"))
plot(p.cloglog)
...
pt.ext <- raster::crop(envs, t.bb)
pt <- predict(m, pt.ext, args=c("outputformat=raw"))
pt.cloglog <- predict(m, pt.ext, args=c("outputformat=cloglog"))
```
...
rmm$authorship$relatedReferences='@article{, title={RMMS: Species’ Range Model Metadata Standards },author={Merow, Cory and Maitner, Brian S. and Owens, Hannah L. and Kass, Jamie M. and Enquist, Brian and Guralnick, Robert},journal={Global Ecology and Biogeography},year={2018}}'
