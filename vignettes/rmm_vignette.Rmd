...
```{r}
rmm1=rangeModelMetadataTemplate(useCase='apObligate') #  Make an empty rmm object containing only the obligate set of fields.
str(rmm1)
```
...
rmm objects can be populated manually by entering data direcetly into the fields, or through the use of several helper functions.  Although rmm object template already contain a number of fields that depend on the specified `useCase`, users can also add new fields as needed.  We are able to provide suggestions of both common fields to add and common values for many fields.
